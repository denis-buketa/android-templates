...
object Client {
    ...
    fun fetchPopularMoviesObservable() {
        ServiceProvider.movieDbService
            .popularMoviesObservable(API_KEY_VALUE, Language.ENGLISH_US.id, 1, Region.US.id)
            .observeOn(AndroidSchedulers.mainThread())
            .subscribeOn(Schedulers.io())
            .subscribe(
                { popularMovies -> Logger.log("onSuccess: $popularMovies") },
                { throwable -> Logger.log("onError: $throwable") })
    }
}